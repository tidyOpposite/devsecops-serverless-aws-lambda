...
| HTTP health validation | `enable_http_validation`, `devsecops health` | Terraform outputs `api_gateway_health_url`. | Deploy job curls `/health` when enabled; failed validation triggers rollback. | API Gateway invokes Lambda through the deployed endpoint. | None. |
| DAST | `enable_dast` | Terraform outputs `api_gateway_invoke_url`. | Deploy job runs OWASP ZAP baseline when enabled; failure triggers rollback. | ZAP scans the public API Gateway invoke URL. | OWASP ZAP baseline. |
...
| `enterprise` | Production-oriented | Snyk | `/health` + DAST | Explicit | On | On | Strongest bundled posture; still review IAM and API auth for your account. |
